...
if [ $status = 0 ] && [ $item = "1" ]; then
  ...
  if [ $input=0 ] && [ ! -z $email ]; then
    ...
    if [ $input=0 ] && [ ! -z $file_name ]; then
      ...
      result=$(whiptail --title "SSH key pair Generator" --msgbox \
  ...
  input=$? 
  if [ $input = 0 ] && [ ! -z $file_name ]; then 
    echo "O nome foi: $file_name"
  else
    echo "cancelado"
  fi
  # echo "Type here your private key file path (eg. ~/.ssh/my_SSH_Key): "
  # read PATH
  # ssh-keygen
  # ~/.ssh/ssh-keygen -y -f $PATH
else
  file_name=$(whiptail \
  --title "SSH key pair Generator" \ 
  --inputbox "Insert file name of your private key located in ~/.ssh/ folder:" 10 60 3>&1 1>&2 2>&3)
  status=$?
  if [ $status = 0 ]; then 
    echo "O nome foi: $file_name"
  fi
